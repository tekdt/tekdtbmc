...
def _update_task(main_window):
    ...
    if has_internet:
        main_window.update_worker.status.emit("Đang kiểm tra phiên bản chương trình...")
        _check_self_update(main_window)
        main_window.update_worker.status.emit("Đã kết nối Internet. Sẵn sàng kiểm tra cập nhật công cụ.")
    else:
        main_window.update_worker.status.emit("Không có kết nối Internet. Sẽ sử dụng các công cụ hiện có.")
    ...
    # Danh sách công cụ bao gồm 7-Zip mới được thêm vào
    tools = [
        ("Fido", config.FIDO_SCRIPT_PATH, _update_fido_script(main_window)),
        ("oscdimg", config.OSCDIMG_EXE, _update_oscdimg_exe(main_window)),
        # Thêm 7-Zip: Sử dụng regex 7z\d+\.msi để khớp với định dạng 7z2501.msi
        ("7-Zip", config.SEVENZIP_EXE, lambda: _update_tool(main_window, "7-Zip", config.SEVENZIP_API_URL, r"7z\d+\.msi", lambda p, d: _extract_and_move(main_window, p, d))),
        ("Ventoy", os.path.join(config.VENTOY_DIR, "Ventoy2Disk.exe"), lambda: _update_tool(main_window, "Ventoy", config.VENTOY_API_URL, r"ventoy-.*-windows\.zip", lambda zip_path, dest_dir: _extract_and_move(main_window, zip_path, dest_dir))),
        ("aria2", config.ARIA2_EXE, lambda: _update_tool(main_window, "aria2", config.ARIA2_API_URL, r"aria2-.*-win-32bit-build.*\.zip", lambda zip_path, dest_dir: _extract_and_move(main_window, zip_path, dest_dir))),
        ("wimlib", config.WIMLIB_EXE, lambda: _update_tool(main_window, "wimlib", config.WIMLIB_URL, r"wimlib-.*-windows.*\.zip", lambda zip_path, dest_dir: _extract_and_move(main_window, zip_path, dest_dir), ssl_verify=False)),
        ("WinCDEmu", config.WINCDEMU_EXE, lambda: _update_tool(main_window, "WinCDEmu", config.WINCDEMU_API_URL, r"PortableWinCDEmu-.*\.exe", lambda dp, dd: _download_and_place_exe(dp, dd, "wcdemu.exe"))),
        ("TekDT_AIS", config.TEKDTAIS_EXE, lambda: _update_tool(main_window, "TekDT_AIS", config.TEKDTAIS_API_URL, r".*\.zip", lambda zip_path, dest_dir: _extract_and_move(main_window, zip_path, dest_dir))),
    ]
...
def _update_tool(main_window, name, api_url, asset_pattern, extract_func, ssl_verify=True):
    try:
        # Hỗ trợ thêm cả .msi trong kiểm tra URL trực tiếp
        is_direct_url = any(api_url.endswith(ext) for ext in [".zip", ".7z", ".msi"])
        if is_direct_url:
            latest_version = os.path.basename(api_url)
        else:
            response = requests.get(api_url)
        ...
        # Đảm bảo sử dụng config.SEVENZIP_DIR nếu công cụ là 7-Zip
        if name == "7-Zip":
            tool_dest_dir = str(config.SEVENZIP_DIR)
        else:
            tool_dest_dir = os.path.join(config.TOOLS_DIR, name)
...
def _extract_and_move(main_window, file_path, dest_dir):
    """
    Hàm trích xuất đa năng: Hỗ trợ .zip (ZipFile) và .msi (msiexec).
    Tự động xử lý cấu trúc thư mục lồng nhau để đưa nội dung về dest_dir.
    """
    ...
    # Giai đoạn 1: Trích xuất dựa trên định dạng file
    if file_path.lower().endswith(".zip"):
        with zipfile.ZipFile(file_path, 'r') as zip_ref:
            zip_ref.extractall(temp_extract_dir)
            
    elif file_path.lower().endswith(".msi"):
        # Sử dụng Administrative Install để trích xuất MSI mà không cài đặt
        # /a: extract, /qb: quiet mode, TARGETDIR: thư mục đích
        cmd = ['msiexec', '/a', os.path.abspath(file_path), '/qb', f'TARGETDIR={os.path.abspath(temp_extract_dir)}']
        subprocess.run(cmd, capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)

    # Giai đoạn 2: Xử lý cấu trúc thư mục (Universal Flattening)
    # Kiểm tra các trường hợp đặc biệt của MSI (thường nằm trong Files/Tên_Ứng_Dụng)
    msi_files_dir = os.path.join(temp_extract_dir, "Files")
    if os.path.exists(msi_files_dir):
        inner_items = os.listdir(msi_files_dir)
        if inner_items:
            # Nếu có folder bên trong Files (như Files/7-Zip), lấy folder đó làm gốc
            inner_path = os.path.join(msi_files_dir, inner_items[0])
            shutil.move(inner_path, dest_dir)
            shutil.rmtree(temp_extract_dir)
            return

    # Kiểm tra trường hợp ZIP có 1 folder gốc duy nhất (Ventoy, aria2)
    extracted_items = os.listdir(temp_extract_dir)
    if not extracted_items:
        shutil.rmtree(temp_extract_dir)
        raise Exception(f"File {os.path.basename(file_path)} không chứa dữ liệu.")
